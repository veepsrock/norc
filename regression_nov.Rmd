...
```{r income reference}
glm <- glm(low_fs ~ relevel(factor(income4_l), ref = 3) + region4_l + asianorigin_l + lang_athome_l, data = df, family = "binomial")
tab_model(glm)
```
## Comparing Age

```{r age reference}
glm <- glm(low_fs ~ relevel(factor(age4_l), ref = 1) + income4_l + region4_l + asianorigin_l + lang_athome_l, data = df, family = "binomial")
tab_model(glm)
```
...
```{r asianorigin reference}
glm <- glm(low_ns ~ relevel(factor(asianorigin_l), ref = 2) + income4_l + region4_l + lang_athome_l, data = df, family = "binomial")
tab_model(glm)
```


```{r asianorigin reference non factored}
glm <- glm(low_fs ~ asianorigin_l + income4_l + region4_l + lang_athome_l, data = df, family = "binomial")
tab_model(glm)
```
