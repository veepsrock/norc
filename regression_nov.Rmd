...
```{r reference q1a}
glm <- glm(not_last ~ relevel(factor(asianorigin), ref = 1), data = df, family = "binomial")
tab_model(glm)
```

```{r reference q1b}
glm <- glm(balance ~ relevel(factor(asianorigin), ref = 1), data = df, family = "binomial")
tab_model(glm)
```

```{r reference q3}
glm <- glm(meal_size ~ relevel(factor(asianorigin), ref = 1), data = df, family = "binomial")
tab_model(glm)
```

```{r reference q4}
glm <- glm(months ~ relevel(factor(asianorigin), ref = 1), data = df, family = "binomial")
tab_model(glm)
```

```{r reference q5}
glm <- glm(eat_less ~ relevel(factor(asianorigin), ref = 1), data = df, family = "binomial")
tab_model(glm)
```

```{r reference q6}
glm <- glm(hungry ~ relevel(factor(asianorigin), ref = 1), data = df, family = "binomial")
tab_model(glm)
```
...
```{r refernce q7}
glm <- glm(hard_to_get ~ relevel(factor(asianorigin), ref = 1), data = df, family = "binomial")
tab_model(glm)
```
